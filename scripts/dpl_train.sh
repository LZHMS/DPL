...
FP=${11} # number of false positive training samples per class

for SEED in {1..3}
do  
    DIR=./output/${DATASET}/${TRAINER}/${CFG}_${SHOTS}shots_EQULE_${CLASS_EQULE}_${CONF_THRESHOLD}_${BLOCK}block_${TAG}/${BETA}beta/nctx${NCTX}_csc${CSC}_ctp${CTP}_fp${FP}/seed${SEED}
    if [ -d "$DIR" ]; then
        ...
        TRAINER.DPL.N_BLOCK ${BLOCK} \
        TRAINER.DPL.BETA ${BETA} \
