...
class POMA(TrainerX):
    ...
    def build_data_loader(self):
        ...
        self.dm = dm
    def calulate_similarity_in_classbag(self):
        self.set_model_mode("eval")
        self.model.eval()
        self.evaluator.reset()
        ...
        for _, batch in tqdm(enumerate(data_loader)):
            input, label, impath = self.parse_batch_test_with_impath(batch)
            _, image_features, _ = self.model(input)
        ...
        print('noise labels', noise_labels.shape)
        print('ground labels', gt_labels.shape)
        print('image_features', image_features.shape)
        print('sstrain_img_paths', sstrain_img_paths.shape)

        for cls in range(self.num_classes):
            cls_index = gt_labels == cls
            cls_labels = noise_labels[cls_index, :]
            cls_images = image_features[cls_index, :]
            cls_impath = sstrain_img_paths[cls_index, :]

            cls_center = torch.mean(cls_images, axis=0)
            for i in range(cls_images.shape[0]):
                cos_sim = F.cosine_similarity(cls_images[i, :].unsqueeze(0), cls_center.unsqueeze(0))
                print(cos_sim)
